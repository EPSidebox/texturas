...
// ═══ TEI IMPORT ═══
function parseTEIImport(xmlText){
  var parser=new DOMParser();var doc=parser.parseFromString(xmlText,"text/xml");
  if(doc.getElementsByTagName("parsererror").length)return null;
  var hasCorp=doc.getElementsByTagName("teiCorpus").length>0;
  var teis=hasCorp?Array.from(doc.getElementsByTagName("TEI")):(doc.documentElement.tagName==="TEI"||doc.documentElement.localName==="TEI")?[doc.documentElement]:[];
  if(!teis.length)return null;
  var results=[];
  teis.forEach(function(tei,idx){
    var titles=tei.getElementsByTagName("title");
    var label=titles.length?titles[0].textContent.trim():"Imported "+(idx+1);
    var bodies=tei.getElementsByTagName("body");if(!bodies.length)return;
    var paras=bodies[0].getElementsByTagName("p");var textParts=[];
    Array.from(paras).forEach(function(p){textParts.push(p.textContent)});
    var text=textParts.join("\n\n").trim();if(!text)return;
    results.push({id:"d"+Date.now()+"_"+idx,label:label,text:text})});
...
function WeaveWordPanel({result,topN,highlightLemma,onClickWord,ngMode,setNgMode,sortBy,setSortBy}){
  ...
  var allActive=!highlightLemma;
  return <div style={{display:"flex",flexDirection:"column",gap:1,minWidth:0,height:"100%"}}>
    <div style={{display:"flex",gap:0,marginBottom:2,border:"1px solid #333",borderRadius:3,overflow:"hidden"}}>{[["1","1"],["2","2"],["3","3"]].map(function(p){return <button key={p[0]} onClick={function(){setNgMode(p[0]);onClickWord(null)}} style={{flex:1,padding:"4px 0",background:ngMode===p[0]?"#bb8fce":"#1a1a1a",color:ngMode===p[0]?"#111":"#666",border:"none",cursor:"pointer",fontSize:10,fontFamily:"monospace",fontWeight:ngMode===p[0]?"bold":"normal"}}>{p[1]}</button>})}</div>
    <button onClick={function(){onClickWord(null)}} style={{padding:"4px 8px",marginBottom:2,background:allActive?"#4ecdc4":"#1a1a1a",color:allActive?"#111":"#999",border:"1px solid "+(allActive?"#4ecdc4":"#333"),borderRadius:4,cursor:"pointer",fontSize:11,fontFamily:"monospace",fontWeight:allActive?"bold":"normal",textAlign:"left"}}>All</button>
    {isUni&&<div style={{display:"flex",gap:0,marginBottom:3,border:"1px solid #333",borderRadius:3,overflow:"hidden"}}>{[["freq","Freq"],["relevance","Relev"]].map(function(p){return <button key={p[0]} onClick={function(){setSortBy(p[0])}} style={{flex:1,padding:"3px 0",background:sortBy===p[0]?"#45b7d1":"#1a1a1a",color:sortBy===p[0]?"#111":"#666",border:"none",cursor:"pointer",fontSize:9,fontFamily:"monospace"}}>{p[1]}</button>})}</div>}
    <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:1}}>{sorted.map(function(pair){var w=pair[0],c=pair[1],rel=isUni?(result.relevanceMap[w]||0):0,isHL=highlightLemma===w,act=allActive||isHL,pr=c>0;
      return <button key={w} onClick={function(){onClickWord(w===highlightLemma?null:w)}} style={{padding:"3px 7px",background:isHL?"#1a2a2a":"#151515",border:"1px solid "+(isHL?"#4ecdc466":"#222"),borderRadius:3,cursor:"pointer",fontSize:10,fontFamily:"monospace",textAlign:"left",display:"flex",flexDirection:"column",gap:1,opacity:pr?1:0.35}}>
        <div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",width:"100%",color:isHL?"#4ecdc4":act&&pr?"#ddd":"#777"}}>{w}</div>
        <div style={{display:"flex",alignItems:"center",gap:4,width:"100%",height:12}}>
          <div style={{flex:1,height:3,background:"#222",borderRadius:1,overflow:"hidden"}}><div style={{width:(c/maxF*100)+"%",height:"100%",background:act&&pr?"#4ecdc4":"#444",borderRadius:1,opacity:act?0.8:0.3}}/></div>
          <span style={{width:24,textAlign:"right",fontSize:9,color:act&&pr?"#aaa":"#555",flexShrink:0}}>{c||""}</span></div>
        {isUni&&<div style={{display:"flex",alignItems:"center",gap:4,width:"100%",height:12}}>
          <div style={{flex:1,height:3,background:"#222",borderRadius:1,overflow:"hidden"}}><div style={{width:(rel/maxR*100)+"%",height:"100%",background:act&&pr?"#45b7d1":"#444",borderRadius:1,opacity:act?0.8:0.3}}/></div>
          <span style={{width:24,textAlign:"right",fontSize:9,color:act&&pr?"#7ab8cc":"#444",flexShrink:0}}>{rel?rel.toFixed(0):""}</span></div>}
      </button>})}</div>
    <div style={{display:"flex",gap:8,marginTop:3,fontSize:9,color:"#888"}}><span><span style={{color:"#4ecdc4"}}>{"\u2014"}</span> freq</span>{isUni&&<span><span style={{color:"#45b7d1"}}>{"\u2014"}</span> relev</span>}</div></div>}
...
function WeaveStandalone(){
  ...
  function handleXMLImport(files){Array.from(files).forEach(function(f){if(!f.name.endsWith(".xml"))return;var reader=new FileReader();reader.onload=function(ev){var imported=parseTEIImport(ev.target.result);if(imported&&imported.length){setDocs(function(d){return d.filter(function(x){return x.text.trim()}).concat(imported)});setActiveInputDoc(imported[0].id)}};reader.readAsText(f)})}
  ...
  return <div style={{background:"#111",color:"#ddd",height:"100vh",fontFamily:"monospace",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{flex:1,padding:"16px 20px",overflowY:"auto"}}>
      ...
      {tab==="input"&&<div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><span style={{fontSize:13,color:"#aaa"}}>Documents ({docs.length})</span><button onClick={addDoc} style={{padding:"4px 12px",background:"#2a2a2a",color:"#4ecdc4",border:"1px solid #444",borderRadius:4,fontSize:12,fontFamily:"monospace",cursor:"pointer"}}>+ Add</button><label style={{padding:"4px 12px",background:"#2a2a2a",color:"#45b7d1",border:"1px solid #444",borderRadius:4,fontSize:12,fontFamily:"monospace",cursor:"pointer"}}>Upload .txt<input type="file" multiple accept=".txt" style={{display:"none"}} onChange={function(ev){handleFiles(ev.target.files)}}/></label><label style={{padding:"4px 12px",background:"#2a2a2a",color:"#f0b27a",border:"1px solid #444",borderRadius:4,fontSize:12,fontFamily:"monospace",cursor:"pointer"}}>Import .xml<input type="file" multiple accept=".xml" style={{display:"none"}} onChange={function(ev){handleXMLImport(ev.target.files)}}/></label></div>
      ...
      {tab==="weave"&&activeWR&&<div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div ref={contentRef} style={{display:"flex",gap:10,alignItems:"stretch",height:540,overflow:"hidden"}}>
          ...
          <div style={{width:160,flexShrink:0,display:"flex",flexDirection:"column"}}><div style={{fontSize:10,color:"#888",marginBottom:3}}>Top {topN} {"\u00B7"} click to feature</div><WeaveWordPanel result={activeWR} topN={topN} highlightLemma={wHighlight} onClickWord={function(w){setWHighlight(w)}} ngMode={wNgMode} setNgMode={setWNgMode} sortBy={wSortBy} setSortBy={setWSortBy}/></div></div>
