...
// ═══ NLP ENGINES (factory functions — no class fields) ═══
var SFX = [
...
function mkPOS() {
  ...
  o.tag = function(w) {
    ...
    for (var i = 0; i < SFX.length; i++) { if (l.endsWith(SFX[i][0])) return SFX[i][1]; }
    return "n";
  };
  return o;
}
...
function mkLem() {
  var o = { exc: null, rl: null, ready: false };
...
function mkSent() {
  var o = { el: null, int: null, vad: null, vdr: null, swn: null, ready: false };
  ...
  o.score = function(lem, pos) {
    ...
    if (o.swn && o.swn[k]) { r.swn = o.swn[k]; }
      ...
      for (var i = 0; i < tries.length; i++) {
        var v = o.swn ? o.swn[lem + "#" + tries[i]] : null;
        if (v) { r.swn = v; break; }
      }
    }
...
function spreadAct(fMap, syn, pos, depth, decay, flow) {
  ...
  var scores = {};
  corpus.forEach(function(l) { scores[l] = fMap[l] || 0; });
  Object.entries(fMap).forEach(function(pair) {
    var l = pair[0], f = pair[1];
    if (f === 0) return;
    ...
    for (var h = 1; h <= depth; h++) {
      var nf = [];
      front.forEach(function(nd) {
        var tgts = syn.directed(nd.w, nd.p, flow);
...
function getNg(ts, n) {
  var g = [];
  for (var i = 0; i <= ts.length - n; i++) g.push(ts.slice(i, i + n).join(" "));
...
function analyzeForWeave(text, eng, topN, winSize, wnDepth, decay, flow) {
  ...
  var allWords = [];
  var renderParas = paras.map(function(para) {
    var parts = [];
    var rx = /([\w'-]+)|(\s+)|([^\w\s'-]+)/g;
    var m;
    while ((m = rx.exec(para)) !== null) {
      if (m[1]) {
        var surface = m[1], lower = surface.toLowerCase();
        var isStop = STOP_WORDS.has(lower) && !NEGATION.has(lower);
        var p = eng.pos.ready ? eng.pos.tag(lower) : "n";
        var lemma = eng.lem.ready ? eng.lem.lemmatize(lower, p) : lower;
        if (!isStop) allWords.push(lemma);
        parts.push({ type: "word", surface: surface, lower: lower, lemma: lemma, pos: p, isStop: isStop });
      } else {
  ...
  var freqMap = _.countBy(allWords);
  var freqPairs = _.chain(freqMap).toPairs().sortBy(1).reverse().value();
  var topWords = freqPairs.slice(0, topN).map(function(f) { return f[0]; });
  var maxFreq = freqPairs[0] ? freqPairs[0][1] : 1;
  var ng2 = getNg(allWords, 2).slice(0, topN);
  var ng3 = getNg(allWords, 3).slice(0, topN);

  // Co-occurrence + Louvain
  var ws = new Set(topWords);
  var mx = {};
  topWords.forEach(function(a) { mx[a] = {}; topWords.forEach(function(b) { mx[a][b] = 0; }); });
  for (var i = 0; i < allWords.length; i++) {
    if (!ws.has(allWords[i])) continue;
    for (var j = Math.max(0, i - winSize); j <= Math.min(allWords.length - 1, i + winSize); j++) {
      if (i !== j && ws.has(allWords[j])) mx[allWords[i]][allWords[j]]++;
    }
  }

  var n = topWords.length;
  var cm = topWords.map(function(_, i) { return i; });
  var tw2 = 0;
  var wt = {};
  topWords.forEach(function(a) { wt[a] = 0; topWords.forEach(function(b) { var w = mx[a][b] || 0; tw2 += w; wt[a] += w; }); });
  tw2 /= 2;
  ...
  if (tw2 > 0) {
    ...
    while (imp && it < 20) {
      ...
      for (var ii = 0; ii < n; ii++) {
        var bc = cm[ii], bg = 0;
        var ucs = Array.from(new Set(cm));
        for (var ci = 0; ci < ucs.length; ci++) {
          var c = ucs[ci];
          if (c === cm[ii]) continue;
          var g = 0;
          for (var jj = 0; jj < n; jj++) {
            if (cm[jj] !== c) continue;
            g += (mx[topWords[ii]][topWords[jj]] || 0) - (wt[topWords[ii]] * wt[topWords[jj]]) / (2 * tw2);
          }
          if (g > bg) { bg = g; bc = c; }
        }
        if (bc !== cm[ii]) { cm[ii] = bc; imp = true; }
      }
    }
  }

  var uComms = Array.from(new Set(cm));
  var commMap = {};
  ...
  var maxRel = Math.max.apply(null, Object.values(relevanceMap).concat([1]));
  var sentCache = {};
  function getSent(lem, p) {
    if (sentCache[lem]) return sentCache[lem];
    var s = eng.sent.ready ? eng.sent.score(lem, p) : {};
    sentCache[lem] = s; return s;
  }
  var enriched = renderParas.map(function(parts) {
    return parts.map(function(t) {
      if (t.type !== "word") return t;
      var s = getSent(t.lemma, t.pos);
...
async function cGet(k) {
  try { var db = await openDB(); return new Promise(function(r) { var t = db.transaction("assets","readonly"); var q = t.objectStore("assets").get(k); q.onsuccess = function() { r(q.result || null); }; q.onerror = function() { r(null); }; }); } catch(e) { return null; }
}
async function cSet(k, v) {
  try { var db = await openDB(); return new Promise(function(r) { var t = db.transaction("assets","readwrite"); t.objectStore("assets").put(v, k); t.oncomplete = function() { r(true); }; t.onerror = function() { r(false); }; }); } catch(e) { return false; }
}
async function loadAsset(key, path, bin, cb) {
  var c = await cGet(key); if (c) return c;
  var b = ASSET_BASE_URL.endsWith("/") ? ASSET_BASE_URL : ASSET_BASE_URL + "/";
  if (cb) cb("Fetching " + path + "...");
  try { var r = await fetch(b + path); if (!r.ok) return null; var d = bin ? await r.arrayBuffer() : await r.json(); await cSet(key, d); return d; } catch(e) { return null; }
}
...
function EmoBars({ emotions, arousal, showEmo, showAro, enabledSlots }) {
  ...
  var active = EMOTIONS.filter(function(e) { return enabledSlots.has(e); });
  var present = new Set(emotions);
...
function WeaveMinimap({ enriched, layers, enabledSlots, maxFreq, maxRel, scrollFrac, viewFrac, onSeek, height }) {
  var cvRef = useRef();
  var flatWords = useMemo(function() {
    var out = [];
  ...
  useEffect(function() {
    ...
    var ctx = cv.getContext("2d"); var h = height || 400;
    cv.width = 80; cv.height = h; ctx.fillStyle = "#0d0d0d"; ctx.fillRect(0, 0, 80, h);
    var total = flatWords.length; if (!total) return;
    var rowH = Math.max(1, Math.min(3, h / Math.ceil(total / 16)));
    var cols = 16, cw = Math.floor(80 / cols);
    var row = 0, col = 0;
    flatWords.forEach(function(t) {
      if (!t) { row++; col = 0; return; }
      var yy = row * rowH, xx = col * cw; if (yy > h) return;
      var c = "#333";
      if (!t.isStop) {
        ...
        else if (layers.emotion) { var fe = (t.emotions || []).filter(function(e) { return enabledSlots.has(e); }); c = fe.length ? EC[fe[0]] : "#555"; }
        else if (layers.relevance) { var rn = maxRel > 0 ? Math.log(1 + t.relevance) / Math.log(1 + maxRel) : 0; var b = Math.round(50 + rn * 200); c = "rgb(" + b + "," + Math.round(b * 1.2) + "," + b + ")"; }
        else if (layers.frequency) { var fn = maxFreq > 0 ? Math.log(1 + t.frequency) / Math.log(1 + maxFreq) : 0; var b2 = Math.round(40 + fn * 180); c = "rgb(" + b2 + "," + b2 + "," + b2 + ")"; }
        else c = "#555";
      } else c = "#222";
    ...
    var mapH = (row + 1) * rowH;
    var vpY = scrollFrac * Math.min(mapH, h), vpH = Math.max(8, viewFrac * Math.min(mapH, h));
  ...
  var handleClick = useCallback(function(e) { var cv = cvRef.current; if (!cv) return; var r = cv.getBoundingClientRect(); onSeek(Math.max(0, Math.min(1, (e.clientY - r.top) / r.height))); }, [onSeek]);
...
function WeaveReader({ enriched, layers, highlightLemma, maxFreq, maxRel, onHover, onClick, enabledSlots, showArousal, scrollRef, onScroll, gridSize }) {
  ...
  var gs = gridSize || 10, cells = gs * gs;
  var totalWords = 0;
  ...
  var binStarts = useMemo(function() {
    var s = new Set(); var pos = 0;
    for (var i = 0; i < cells; i++) { s.add(pos); pos += i < extra ? base + 1 : base; }
    return s;
  }, [totalWords, cells, base, extra]);
  function binLabel(wi) {
    var pos = 0;
    for (var i = 0; i < cells; i++) { var sz = i < extra ? base + 1 : base; if (wi >= pos && wi < pos + sz) return "[" + (Math.floor(i / gs) + 1) + "," + (i % gs + 1) + "]"; pos += sz; }
    return null;
  }
  var wordIdx = 0;
  return (
        ...
        return (
          <div key={pi} style={{ marginBottom:20 }}>
            {para.map(function(t, ti) {
              ...
              var isWord = t.lower && t.lower.length > 1;
              var wi = isWord ? wordIdx : -1;
              if (isWord) wordIdx++;
              var showMarker = isWord && binStarts.has(wi);
              var marker = showMarker ? binLabel(wi) : null;
              if (t.isStop) return (
...
// ═══ WORD PANEL (rewritten — Vellum-matching) ═══
function WeaveWordPanel({ result, topN, highlightLemma, onClickWord, ngMode, setNgMode, sortBy, setSortBy }) {
  var isUni = ngMode === "1";
  var raw = isUni ? result.freqPairs.slice(0, topN) : ngMode === "2" ? result.ng2.slice(0, topN) : result.ng3.slice(0, topN);
  ...
  var maxF = raw[0] ? raw[0][1] : 1;
  var maxR = isUni ? Math.max.apply(null, raw.map(function(x) { return result.relevanceMap[x[0]] || 0; }).concat([1])) : 1;
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:1, minWidth:0, height:"100%" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:1 }}>
        {sorted.map(function(pair) {
          var w = pair[0], c = pair[1], rel = isUni ? (result.relevanceMap[w] || 0) : 0, isHL = highlightLemma === w;
          return (
  ...
  return (
    <div style={{ display:"inline-grid", gridTemplateColumns:"repeat(3,10px)", gap:1 }}>
      {EMO_LAYOUT.map(function(item, i) {
        var key = item.emo || "center", on = enabledSlots.has(key), col = item.emo ? EC[item.emo] : "#ffffff";
        return <div key={i} onClick={function() { setEnabledSlots(function(prev) { var n = new Set(prev); if (n.has(key)) n.delete(key); else n.add(key); return n; }); }} title={item.emo || "relevance"} style={{ width:10, height:10, borderRadius:1, background: on ? col : "#555", opacity: on ? 1 : 0.5, cursor:"pointer", border: "1px solid " + (on ? col + "66" : "#444") }} />;
      })}
    </div>
  );
}
...
function WeaveStandalone() {
  var _docs = useState([{ id:"d1", label:"Document 1", text:"" }]);
  var docs = _docs[0], setDocs = _docs[1];
  ...
  var _fl = useState("bi"); var flow = _fl[0], setFlow = _fl[1];
  var winSize = 5;
  var _wpd = useState({}); var weavePerDoc = _wpd[0], setWeavePerDoc = _wpd[1];
  ...
  var readerRef = useRef();
  var _sf = useState(0); var scrollFrac = _sf[0], setScrollFrac = _sf[1];
  var _vf = useState(1); var viewFrac = _vf[0], setViewFrac = _vf[1];
  var _ch = useState(400); var contentH = _ch[0], setContentH = _ch[1];

  var handleReaderScroll = useCallback(function() {
    var el = readerRef.current; if (!el) return;
  ...
  var handleMinimapSeek = useCallback(function(frac) {
    var el = readerRef.current; if (!el) return;
  ...
  useEffect(function() {
    ...
    var ro = new ResizeObserver(function(en) { setContentH(en[0].contentRect.height); });
  ...
  useEffect(function() {
    ...
    (async function() {
      var e = eng.current;
      var pd = await loadAsset("w-p", "wordnet/pos-lookup.json", false, setMsg); if (pd && !cancelled) e.pos.load(pd);
      var ld = await loadAsset("w-l", "wordnet/lemmatizer.json", false, setMsg); if (ld && !cancelled) e.lem.load(ld);
      var sd = await loadAsset("w-s", "wordnet/synsets.json", false, setMsg); if (sd && !cancelled) e.syn.load(sd);
      var el = await loadAsset("l-e", "lexicons/nrc-emolex.json", false, setMsg); if (el && !cancelled) e.sent.lEl(el);
      var ni = await loadAsset("l-i", "lexicons/nrc-intensity.json", false, setMsg); if (ni && !cancelled) e.sent.lInt(ni);
      var nv = await loadAsset("l-v", "lexicons/nrc-vad.json", false, setMsg); if (nv && !cancelled) e.sent.lVad(nv);
      var va = await loadAsset("l-d", "lexicons/vader.json", false, setMsg); if (va && !cancelled) e.sent.lVdr(va);
      var sw = await loadAsset("l-s", "lexicons/sentiwordnet.json", false, setMsg); if (sw && !cancelled) e.sent.lSwn(sw);
      if (!cancelled) setMsg("");
    })();
  ...
  var analyzedIds = Object.keys(weavePerDoc);
  ...
  var activeWR = wActiveDoc ? weavePerDoc[wActiveDoc] : null;
  var curDoc = docs.find(function(d) { return d.id === activeInputDoc; });
  var hasMarkers = curDoc && curDoc.text && curDoc.text.indexOf("---DOC") !== -1;
  ...
  return (
    <div style={{ background:"#111", color:"#ddd", minHeight:"100vh", fontFamily:"monospace", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"flex", borderBottom:"1px solid #2a2a2a" }}>
        {[{ id:"input", l:"Input" }, { id:"weave", l:"Weave" }].map(function(t) {
          return <button key={t.id} onClick={function() { setTab(t.id); }} style={{ padding:"10px 14px", background: tab === t.id ? "#1a1a1a" : "transparent", color: tab === t.id ? "#4ecdc4" : "#888", border:"none", borderBottom: tab === t.id ? "2px solid #4ecdc4" : "2px solid transparent", cursor:"pointer", fontSize:12, fontFamily:"monospace" }}>{t.l}</button>;
        })}
      </div>
              ...
              return (
                <div key={d.id}>
                  <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:8 }}>
                    ...
                    {hasMarkers && <button onClick={function() { var p = parseSep(d.text); if (p) { setDocs(p); setActiveInputDoc(p[0].id); } }} style={{ padding:"6px 14px", background:"#2a2a1a", color:"#f0b27a", border:"1px solid #f0b27a44", borderRadius:4, cursor:"pointer", fontSize:11, fontFamily:"monospace" }}>Split</button>}
                  </div>
