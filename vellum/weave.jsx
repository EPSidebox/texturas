...
function WeaveReader({enriched,layers,Lemma,maxFreq,maxRel,onHover,onClick,enabledSlots,showArousal,scrollRef,onScroll,gridSize}){
  ...
  return <div ref={scrollRef} onScroll={onScroll} style={{flex:1,overflowY:"auto",padding:"24px 32px",lineHeight:2.6,wordSpacing:"0.06em"}}>{enriched.map(function(para,pi){return <div key={pi} style={{marginBottom:20}}>{para.map(function(t,ti){if(t.type!=="word")return <span key={ti} style={{fontFamily:"monospace"}}>{t.surface}</span>;var isWord=t.lower&&t.lower.length>1;var wi=isWord?wordIdx:-1;if(isWord)wordIdx++;var showMarker=isWord&&binStarts.has(wi);var marker=showMarker?binLabel(wi):null;
    ...
    if(Lemma&&t.lemma===Lemma){s.outline="2px solid #4ecdc4";s.outlineOffset="3px";s.borderRadius="3px";s.textShadow="0 0 8px #4ecdc466";if(!s.backgroundColor)s.backgroundColor="#4ecdc40d"}
...
function WeaveWordPanel({result,topN,Lemma,onClickWord,ngMode,setNgMode,sortBy,setSortBy}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:1,minWidth:0,height:"100%"}}>
    ...
    <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:1}}>{sorted.map(function(pair){var w=pair[0],c=pair[1],rel=isUni?(result.relevanceMap[w]||0):0,isHL=Lemma===w,act=allActive||isHL,pr=c>0;
      return <button key={w} onClick={function(){onClickWord(w===Lemma?null:w)}} style={{padding:"3px 7px",background:isHL?"#1a2a2a":"#151515",border:"1px solid "+(isHL?"#4ecdc466":"#222"),borderRadius:3,cursor:"pointer",fontSize:10,fontFamily:"monospace",textAlign:"left",display:"flex",flexDirection:"column",gap:1,opacity:pr?1:0.35}}>
...
function WeaveStandalone(){
  ...
  var _wh=useState(null);var w=_wh[0],setW=_wh[1];
  ...
  return <div style={{background:"#111",color:"#ddd",minHeight:"100vh",fontFamily:"monospace",display:"flex",flexDirection:"column"}}>
    ...
    {tab==="weave"&&analyzedIds.length>0&&<div style={{display:"flex",gap:4,padding:"8px 20px",borderBottom:"1px solid #2a2a2a",background:"#151515"}}>{validDocs.filter(function(d){return weavePerDoc[d.id]}).map(function(d){return <button key={d.id} onClick={function(){setWActiveDoc(d.id);setW(null)}} style={{padding:"4px 12px",borderRadius:3,border:"1px solid "+(wActiveDoc===d.id?"#45b7d1":"#333"),background:wActiveDoc===d.id?"#45b7d1":"#1a1a1a",color:wActiveDoc===d.id?"#111":"#888",fontSize:11,fontFamily:"monospace",cursor:"pointer"}}>{d.label}</button>})}</div>}
    <div style={{flex:1,padding:"16px 20px",overflowY:"auto"}}>
      ...
      {tab==="weave"&&activeWR&&<div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div ref={contentRef} style={{display:"flex",gap:10,alignItems:"stretch",height:540,overflow:"hidden"}}>
          <WeaveReader enriched={activeWR.enriched} layers={wLayers} Lemma={w} maxFreq={activeWR.maxFreq} maxRel={activeWR.maxRel} onHover={function(t,x,y){setWHovTok(t);setWHovPos({x:x,y:y})}} onClick={function(lem){setW(function(prev){return prev===lem?null:lem})}} enabledSlots={enabledSlots} showArousal={showArousal} scrollRef={readerRef} onScroll={handleReaderScroll} gridSize={gridSize}/>
