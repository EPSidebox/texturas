...
function VellumGrid({bins,scale,showSize,showColor,showVol,gridSize,normMaxes,label,ngMode,fixedWidth,fixedHeight,topNWords}){
  ...
  useEffect(()=>{const S=stRef.current;if(!S.boxes||S.boxes.length!==bins.length)return;
    ...
    S.boxes.forEach((mesh,i)=>{const bin=bins[i];if(bin.empty){mesh.visible=false;return}mesh.visible=true;
      ...
      if(showVol){const h=Math.max(.08,Math.abs(volV[i])*(gs<=10?5:gs<=20?3.5:2));mesh.scale.set(sz,bin.dimmed?.04:h,sz);mesh.position.y=bin.dimmed?.02:h/2}
  ...
  return (
    <div style={{position:"relative",width:"100%",height:"100%"}}>
      ...
      {showTip&&(
        <div onClick={e=>{if(isPinned){e.stopPropagation();setPinned(null)}}} style={{position:"absolute",top:8,left:8,padding:"10px 14px",background:"#111111f0",border:`1px solid ${isPinned?"#4ecdc4":"#333"}`,borderRadius:6,fontFamily:"monospace",fontSize:10,pointerEvents:isPinned?"auto":"none",maxWidth:280,maxHeight:isPinned?300:180,overflowY:isPinned?"auto":"hidden",zIndex:3,cursor:isPinned?"pointer":"default"}}>
          <div style={{color:"#4ecdc4",fontSize:12,marginBottom:4}}>
            ...
            {isPinned&&<span style={{color:"#666",fontSize:9,marginLeft:6}}>· click to dismiss</span>}
          </div>
          ...
          {showVol&&<div style={{color:"#888"}}>A: <span style={{color:"#f7dc6f"}}>{activeBin.arousal.toFixed(3)}</span></div>}
          {activeBin.fullText&&(
